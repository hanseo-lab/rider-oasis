...
public class RouteService {
    ...
    public RouteResponse createRoute(RouteRequest request) {
        ...
        // FASTEST ?êÎäî COMFORT Î™®Îìú??TMAP ?¨Ïö© (?§Ï†ú ?ÑÎ°ú)
        if (request.getRouteType() == Route.RouteType.FASTEST || request.getRouteType() == Route.RouteType.COMFORT) {
            ...
            // SHADE_OPTIMIZED ?±Ï? Í∏∞Ï°¥ A* ?åÍ≥†Î¶¨Ï¶ò ?¨Ïö©
            AStarPathfinder.PathResult pathResult = pathfinder.findOptimalPath(
                    request.getStartLat(), request.getStartLng(),
                    request.getEndLat(), request.getEndLng(),
                    user.getPreferShade(),
        ...
        Route route = Route.builder()
                .user(user.getId() != null ? user : null) // IDÍ∞Ä ?àÎäî ?¨Ïö©?êÎßå ?§Ï†ï
                ...
                .shelterCount(0) // TODO: Í≤ΩÎ°ú ???Ä?ºÏãú????Í≥ÑÏÇ∞
                .routeType(request.getRouteType())
                .build();

        // Î°úÍ∑∏?∏Ìïú ?¨Ïö©?êÎßå DB???Ä??
    ...
    public RouteResponse getRouteById(Long routeId) {
        Route route = routeRepository.findById(routeId)
                .orElseThrow(() -> new RuntimeException("Í≤ΩÎ°úÎ•?Ï∞æÏùÑ ???ÜÏäµ?àÎã§."));

        // Í∂åÌïú Ï≤¥ÌÅ¨
        User user = getCurrentUser();
        if (!route.getUser().getId().equals(user.getId())) {
            throw new RuntimeException("?ëÍ∑º Í∂åÌïú???ÜÏäµ?àÎã§.");
        }
    ...
    private User getCurrentUser() {
        ...
        if (authentication == null || !authentication.isAuthenticated()
            ...
            // ?µÎ™Ö ?¨Ïö©?êÎ? ?ÑÌïú Í∏∞Î≥∏ User Í∞ùÏ≤¥ Î∞òÌôò (ID??null)
            return User.builder()
                    .username("anonymous")
        ...
        // emailÎ°?Î®ºÏ? Ï°∞Ìöå, ?ÜÏúºÎ©?username?ºÎ°ú Ï°∞Ìöå
        return userRepository.findByEmail(emailOrUsername)
                .orElseGet(() -> userRepository.findByUsername(emailOrUsername)
                        .orElseThrow(() -> new RuntimeException("?¨Ïö©?êÎ? Ï∞æÏùÑ ???ÜÏäµ?àÎã§: " + emailOrUsername)));
    }
    ...
    private String convertToGeoJson(AStarPathfinder.PathResult pathResult) {
        try {
            ...
            throw new RuntimeException("GeoJSON Î≥Ä???§Ìå®", e);
        }
    }
