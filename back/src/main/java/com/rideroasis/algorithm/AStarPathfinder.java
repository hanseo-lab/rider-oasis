...
import com.rideroasis.service.ClimateAPIService;
...
public class AStarPathfinder {
    ...
    private final ClimateAPIService climateAPI;
     ...
     * 실제 경기기후 API 데이터 활용 (공원, 녹지 데이터)
     */
    private double calculateShadeScore(double lat, double lng) {
        try {
            return climateAPI.getShadeScore(lat, lng, 0.5); // 반경 500m
        } catch (Exception e) {
            // API 실패 시 기본값 반환
            return 0.5;
        }
     ...
     * 실제 경기기후 API 데이터 활용 (건물 탄소 배출량)
     */
    private double calculateHeatScore(double lat, double lng) {
        try {
            return climateAPI.getHeatScore(lat, lng, 0.5); // 반경 500m
        } catch (Exception e) {
            // API 실패 시 기본값 반환
            return 0.5;
        }
